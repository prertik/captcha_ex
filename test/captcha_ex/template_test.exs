defmodule CaptchaExTemplateTest do
  ...
  test "supplying a public key in options to display/1 overrides it in the g-recaptcha-div" do
    template_string =
      CaptchaEx.Template.display(public_key: "override_test_public_key")
  ...
  test "supplying noscript option displays the noscript fallback" do
    template_string = CaptchaEx.Template.display(noscript: true)
  ...
  test "supplying a hl in options to display/1 overrides it in the script tag" do
    template_string = CaptchaEx.Template.display(hl: "en")
  ...
  test "supplying a onload in options to display/1 adds it to the script tag" do
    template_string1 = CaptchaEx.Template.display(onload: "onLoad")
    template_string2 = CaptchaEx.Template.display(onload: "onLoad", hl: "en")
  ...
  test "supplying a invisible recaptcha on option size equal invisible" do
    template_string = CaptchaEx.Template.display(size: "invisible")
    assert template_string =~ "grecaptcha.execute()"

    template_string = CaptchaEx.Template.display(size: "compact")
  ...
  test "supplying an option to change the callback even if you are using invisible recaptcha" do
    template_string = CaptchaEx.Template.display()
    ...
    template_string =
      CaptchaEx.Template.display(size: "invisible", callback: "myCallback")
